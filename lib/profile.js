...
profile.prototype = {
  ...
  // USER INFO
  'getinfo' : function(req, res, params, db) {
    if (typeof params.uid == 'undefined') {
      res.send('please provide uid');
    }
    else {
      var query = 'SELECT * FROM users WHERE uid=?';
      return db.query(query, [params.uid], function (err, results, fields) {
        if (err) {
...
//          });
  ...
  'edituser' : function(params) {
    ...
    if (params.uid == '') {
      var query = 'INSERT INTO users (lname,fname,nickname,email,password,cdate) VALUES ("'
          + params.info.lname
          + '","'
          + params.info.fname
          + '","'
